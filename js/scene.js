...
async function downloadSceneAsZip() {
    ...
    // Fetch and include the static README.txt file
    try {
        const readmeResponse = await fetch('README.txt');
        if (!readmeResponse.ok) {
            throw new Error(`Failed to fetch README.txt: ${readmeResponse.status} ${readmeResponse.statusText}`);
        }
        const readmeContent = await readmeResponse.text();
        console.log('Successfully loaded README.txt from file');
        zip.file("README.txt", readmeContent);
    } catch (error) {
        console.error('Failed to load README.txt:', error);
    }
    ...
    creditsContent += "\n";
    creditsContent += "Generated by Hero Me Builder - https://heromebuilder.com\n";
    ...
    if (typeof gtag !== 'undefined') {
        gtag('event', 'download_build', {
            'event_category': 'Build',
            'event_label': 'Hero Me Assembly ZIP',
            'value': 1
        });
    }
}
...
async function loadBuildFromZip() {
    ...
    input.onchange = async (e) => {
        ...
        try {
            ...
            // Track load build event in Google Analytics
            if (typeof gtag !== 'undefined') {
                gtag('event', 'load_build', {
                    'event_category': 'Build',
                    'event_label': 'Build Loaded from ZIP/JSON',
                    'value': 1
                });
            }
...
async function loadAttachment(attachment) {
    ...
    if (attachment.parentAttachment) {
        ...
        // If not found and this is a wing or cable tower attachment, use filename to determine left/right
        if (!targetPoint && (attachment.attachmentPoint.type === 'wing' || attachment.attachmentPoint.type === 'cabletower')) {
